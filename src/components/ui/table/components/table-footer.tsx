...
const TableFooter = ({
    ...
    maxRowOptionConfig?: optionItemType[];
}) => {
    const { screenSize } = useContext(GlobalContext) as _GlobalContextType;
    ...
    const columnList = useDeepCompareMemo<optionItemType[]>(() => {
        const tamp: optionItemType[] = column
            .filter(
                (i) =>
                    i.key !== "#checkbox" && i.key !== "#expandable" && !i.isAlwaysShow
            )
            .map((i) => {
                return {
                    id: i.key,
    ...
    return (
        <div
            ...
            style={{
                gridTemplateColumns:
                    screenSize === "laptop" ? "1fr 1fr 1fr" : "1fr max-content",
            }}
        >
            {tableConfig && screenSize === "laptop" && (
                <div className="footer-left">
                    {!isLoading ? (
                        <p>
                            <span>
                                Showing {footerNuberRow[0]}-{footerNuberRow[1]} of{" "}
                                {tableConfig?.totalData}{" "}
                                {selectedRowCount > 0 && `, ${selectedRowCount} rows selected`}
                            </span>
                        </p>
                    ) : (
                        <Spinner size="small" />
                ...
                {tableConfig && onClickPagination && (
                    <>
                        ...
                        <p style={{ width: "max-content" }}>
                            Page {tableConfig.currentPage} / {tableConfig.countPage}
                        </p>
                        <IconButton
            ...
            <div className="footer-right">
                {screenSize === "mobile" ? (
                    <>
                        ...
                        >
                            <TableSettingMobile
                                ...
                                maxRowOption={maxRowOptionConfig ?? maxRowOption}
                                column={column}
                                setColumn={setColumn}
                    ...
                    <DropdownMenu
                        ...
                        options={maxRowOptionConfig ?? maxRowOption}
                        onClick={(id) => {
...
const TableSettingMobile = ({
    ...
    return (
        <div>
            {tableConfig && onSelectMaxRow && (
                <div>
                    <p className="section-title">Max Row</p>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
                        {maxRowOptionConfig
                            ? maxRowOptionConfig.map((i) => (
                                <Button
                                    key={i.id}
                                    txtLabel={i.txtLabel}
                                    isSelected={`${tableConfig.maxRow}` === i.id}
                                    iconBefore={
                                        `${tableConfig.maxRow}` === i.id ? (
            ...
            <div style={{ marginTop: "var(--space-250)" }}>
                <p className="section-title">Column Order</p>
                <div ref={columnRef} className="list-column-box">
                    {column.map((headerData) => (
                        <div
                            key={headerData.key}
                            className={clsx("item-column", {
                                ["no-swap"]:
